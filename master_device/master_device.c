...
static int master_mmap(struct file *file, struct vm_area_struct *vas);//mmap
...
static int master_mmap(struct file *file, struct vm_area_struct *vas){
	if (remap_pfn_range(vas, vas->vm_start, vas->vm_pgoff, vas->vm_end - vas->vm_start, vas->vm_page_prot))
		return -EIO;
